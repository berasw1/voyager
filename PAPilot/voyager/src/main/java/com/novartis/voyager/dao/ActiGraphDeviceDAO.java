...
public class ActiGraphDeviceDAO {
	...
	//		Long qclJsonLogId = saveActivGraphQCLJsonData(actigraphJsonData, assetId, connection);
			...
			if(records != null && recordslength > 0 ){
				if (recordslength < 3){
					saveActiGraphRecords(records, deviceDetailsId, connection, serialNumber);
				}else{
					saveActiJobTracker(qclJsonLogId, deviceDetailsId, connection, serialNumber);
				}
			}
	...
	/**
	 * The method to store the JSON data of Actigraph device directly in database column.
	 * 
	 * @param activGraphQclJsonData
	 * @param uuid
	 * @return
	 * @throws SQLException
	 */
	public Long saveActivGraphQCLJsonData(String activGraphQclJsonData, String uuid) throws SQLException{
		PreparedStatement pStatement = null;
		Connection connection = null;
		LOGGER.debug("Entering ActiGraphDeviceDAO.saveActivGraphQCLJsonData method.");
		try{
			connection = connectionPool.getConnection();
			connection.setAutoCommit(false);
		...
		//	myClob.setString(1, "");
			...
			clobWriter.write(activGraphQclJsonData);
			
			 sb.append(activGraphQclJsonData);
			 myClob.setString(1, sb.toString());
			 
			 LOGGER.debug("size of myclob :: "+myClob.length());
			...
			LOGGER.debug("Leaving ActiGraphDeviceDAO.saveActivGraphQCLJsonData method.");
		}
	}
	...
	public boolean saveActiGraphRecords(Record[] records, Long deviceDetailsId,
		...
		PreparedStatement pStatement = null;
		LOGGER.debug("Entering ActiGraphDeviceDAO.saveActiGraphRecords method.");
		int recordslength = records.length;
		LOGGER.info("Number of records : "+recordslength+" for device details id "+deviceDetailsId);
		try{
			for(Record record : records){
				...
				if (recordslength <3 && record.activityData != null && record.activityData.value != null
						&& record.activityData.value.length > 0){
					saveActiGraphActivityData(record.activityData, recordId, connection, serialNumber);
				}
				
			}
			LOGGER.debug("Leaving ActiGraphDeviceDAO.saveActiGraphRecords method.");
			return true;
		}catch(Exception e){
	...
	private boolean saveActiJobTracker(Long qclJsonLogId, Long deviceDetailsId, Connection connection, String serialNumber) throws SQLException{
		PreparedStatement pStatement = null;
		LOGGER.debug("Entering ActiGraphDeviceDAO.saveActiJobTracker method.");
		try{
		 	pStatement = connection.prepareStatement(QueryConstants.VoyagerActivGraphSqlConstants.ACTIGRAPH_JOB_TRACKER_INSERT_QRY);

				pStatement.setLong(1, deviceDetailsId);
				pStatement.setString(2, serialNumber);
				pStatement.setLong(3, qclJsonLogId);
			
			if(pStatement.executeUpdate() == 0)
				throw new SQLException();
			LOGGER.debug("Leaving ActiGraphDeviceDAO.saveActiJobTracker method.");
			return true;
		}catch(Exception e){
			LOGGER.error("Exception in storing saveActiJobTracker data ",e);
			throw new SQLException();
		}
	}
